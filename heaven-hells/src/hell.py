...
class Hell():
    def __init__(self, window, color, col, row):
        self.window = window
        self.col = col - 1
        self.row = row - 1
        self.window.matrix[self.row, self.col] = 3
        self.circle = self.window.canvas.create_oval(
            self.col*100, self.row*100, (self.col+1)*100, (self.row+1)*100, fill=color)
